...
| Write to a bridge's inbox without a Ghost Key | Before storing an entry, every peer checks that its certificate names the key that signed it, then that signature, then the certificate's chain to the master key, cheapest first: a forged entry costs no RSA check, and a forged certificate one per message |

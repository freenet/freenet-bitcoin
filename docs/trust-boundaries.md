...
| Grow the inbox's removals past what peers can hold | Only the bridge signs removals, one batch per height naming only what it has read; it stops reading at `REMOVAL_BUDGET`, half the hard bound every peer enforces (`MAX_REMOVED`), and gives each Ghost Key a 64th of that. Spending it takes 64 Ghost Keys each sending 64 requests within about half an hour, and makes new requests wait for the floor |

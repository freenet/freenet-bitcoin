...
| Write to a bridge's inbox without a Ghost Key | Before storing an entry, every peer checks that its certificate names the key that signed it, then that signature, then the certificate's chain to the master key, cheapest first: a forged entry costs no RSA check, and a message at most one per distinct genuine certificate it carries plus one for a forged certificate |
...
| Flood a bridge's inbox | At most 2 waiting requests per Ghost Key and 128 in all, newest kept; a request gives its place back once read, and the bridge reads each Ghost Key only up to a 64th of its removal budget. Holding the inbox therefore takes 64 Ghost Keys each sending about 66 requests every five blocks, about 50 minutes, dated at the top of the window, which is the same act as spending the removal budget. A delta carrying more than 2 entries from one Ghost Key is refused before anything is checked. **Not stopped outright.** See `MAX_ENTRIES` |
| Grow the inbox's removals past what peers can hold | Only the bridge signs removals, one batch per height naming only what it has read; it stops reading at `REMOVAL_BUDGET`, half the hard bound every peer enforces (`MAX_REMOVED`), and gives each Ghost Key a 64th of that. Spending it takes 64 Ghost Keys each having 64 requests read within five blocks, about 50 minutes (about 66 sent each, counting the two each leaves waiting to hold the inbox), and makes new requests wait for the floor |
| Make a removal of your own entry take someone else's with it | A removal names 8 bytes of the entry's digest, so it would take an entry sharing those with a pending one: about 2^57 tries when any of up to 128 pending entries will do, against entries that live about half an hour |
